...
class Metar(object):
    ...
    def json_report(self):
        """
        Return a json_report version of the decoded report.
        """
        lines = {}
        lines["station"] = self.station_id
        if self.type:
            lines["type"] = self.report_type()
        if self.time:
            lines["time"] = self.time.ctime()
        if self.temp:
            lines["temperature"] = self.temp.string("C")
        if self.dewpt:
            lines["dew point"] = self.dewpt.string("C")
        if self.wind_speed:
            lines["wind"] = self.wind()
        if self.wind_speed_peak:
            lines["peak wind"] = self.peak_wind()
        if self.wind_shift_time:
            lines["wind shift"] = self.wind_shift()
        if self.vis:
            lines["visibility"] = self.visibility()
        if self.runway:
            lines["visual range"] = self.runway_visual_range()
        if self.press:
            lines["pressure"] = self.press.string("mb")
        if self.weather:
            lines["weather"] = self.present_weather()
        if self.sky:
            lines["sky"] = self.sky_conditions("\n     ")
        if self.press_sea_level:
            lines["sea-level pressure"] = self.press_sea_level.string("mb")
        if self.max_temp_6hr:
            lines["6-hour max temp"] = str(self.max_temp_6hr)
        if self.max_temp_6hr:
            lines["6-hour min temp"] = str(self.min_temp_6hr)
        if self.max_temp_24hr:
            lines["24-hour max temp"] = str(self.max_temp_24hr)
        if self.max_temp_24hr:
            lines["24-hour min temp"] = str(self.min_temp_24hr)
        if self.precip_1hr:
            lines["1-hour precipitation"] = str(self.precip_1hr)
        if self.precip_3hr:
            lines["3-hour precipitation"] = str(self.precip_3hr)
        if self.precip_6hr:
            lines["6-hour precipitation"] = str(self.precip_6hr)
        if self.precip_24hr:
            lines["24-hour precipitation"] = str(self.precip_24hr)
        if self.ice_accretion_1hr:
            lines["1-hour Ice Accretion"] = str(self.ice_accretion_1hr)
        if self.ice_accretion_3hr:
            lines["3-hour Ice Accretion"] = str(self.ice_accretion_3hr)
        if self.ice_accretion_6hr:
            lines["6-hour Ice Accretion"] = str(self.ice_accretion_6hr)
